...
export async function generateMetadata({ params }) {
  ...
  if (!cam) {
    return {
      title: 'Camera Not Found | BangkokCams.com',
    };
  }

  return {
    title: `${cam.name} Live BMA CCTV & Traffic Cam | BangkokCams.com`,
    description: `Real-time street traffic, weather, and crowd conditions at ${cam.name} (${cam.name_th}) in Bangkok, Thailand. Official municipal BMA traffic CCTV surveillance feed.`,
    openGraph: {
      title: `${cam.name} Live CCTV | BangkokCams.com`,
      description: `Live street webcam and traffic feed at ${cam.name}, Bangkok.`,
      url: `https://bangkokcams.com/cams/${cam.slug}/`,
      type: 'video.other',
    },
  };
}
...
export default async function CamPage({ params }) {
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'Place',
        '@id': `https://bangkokcams.com/cams/${cam.slug}/#place`,
        name: cam.name,
        description: `Traffic and weather monitoring camera at ${cam.name} (${cam.name_th}), Bangkok.`,
        geo: {
          '@type': 'GeoCoordinates',
          latitude: cam.lat,
          longitude: cam.lng,
      ...
      {
        '@type': 'VideoObject',
        name: `${cam.name} Live BMA CCTV Feed`,
        description: `Official municipal BMA traffic CCTV live surveillance feed from ${cam.name}, Bangkok, Thailand.`,
        thumbnailUrl: 'https://bangkokcams.com/og-image.jpg',
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: 'https://bangkokcams.com/?view=map',
          },
          {
            '@type': 'ListItem',
            position: 3,
            name: cam.name,
            item: `https://bangkokcams.com/cams/${cam.slug}/`,
          },
        ],
      },
    ],
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-white">Airport ➔ Bangkok Hotel Direct</span>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Pre-book your private airport taxi door-to-door from Suvarnabhumi (BKK) or Don Mueang (DMK).
                </p>
              </div>
            </div>
            <a
