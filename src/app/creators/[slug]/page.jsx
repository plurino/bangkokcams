...
const ZONE_LABELS = {
  soi_4: 'Sukhumvit Soi 4 & Nana Plaza',
  cowboy: 'Soi Cowboy & Asok',
  soi_11: 'Sukhumvit Soi 11 Nightlife',
  khaosan: 'Khaosan Road & Old City',
  rca: 'RCA & Clubbing Strip',
  silom: 'Silom, Patpong & Sala Daeng',
  thonglor: 'Thonglor & Ekkamai Trendy Strip',
  chinatown: 'Yaowarat Chinatown',
  siam: 'Siam & Central World Hub',
  expat_guide: 'Bangkok Metropolitan & Expat Life'
};
...
export async function generateMetadata({ params }) {
  ...
  if (!creator) {
    return {
      title: 'Creator Not Found | BangkokCams.com',
    };
  }

  const cleanBioSnippet = creator.bio_seo
    ? creator.bio_seo.slice(0, 155).trim() + '...'
    : `Explore latest videos and travel guide for ${creator.name} in Bangkok, Thailand.`;

  return {
    title: `${creator.name} | Bangkok Creator Guide & Latest Videos`,
    description: `${cleanBioSnippet} 4K street walks, nightlife guides, and neighborhood tours across Bangkok.`,
    openGraph: {
      title: `${creator.name} | Bangkok Creator Guide & Latest Videos`,
      description: cleanBioSnippet,
      url: `https://bangkokcams.com/creators/${creator.slug}/`,
      type: 'profile',
    },
  };
}
...
export default async function CreatorProfilePage({ params }) {
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'Person',
        '@id': `https://bangkokcams.com/creators/${creator.slug}/#person`,
        name: creator.name,
        description: creator.bio_seo || `Content creator covering Bangkok, Thailand.`,
        url: `https://bangkokcams.com/creators/${creator.slug}/`,
        sameAs: [primaryChannelUrl].filter(Boolean),
        knowsAbout: ['Bangkok', 'Thailand Tourism', 'Nightlife', 'Travel Vlogging'],
      },
      {
        '@type': 'BreadcrumbList',
        itemListElement: [
          {
            ...
            position: 1,
            name: 'Home',
            item: 'https://bangkokcams.com/',
          },
          {
            '@type': 'ListItem',
            position: 2,
            name: 'Creators Directory',
            item: 'https://bangkokcams.com/creators/',
          },
          {
            '@type': 'ListItem',
            position: 3,
            name: creator.name,
            item: `https://bangkokcams.com/creators/${creator.slug}/`,
          },
        ],
      },
    ],
    ...
    <div className="min-h-screen w-full bg-canvas text-slate-100 flex flex-col">
        ...
        <section className="bg-surface border border-borderDark rounded-2xl p-6 sm:p-8 flex flex-col gap-4 shadow-md">
          <div className="flex items-center gap-2 border-b border-borderDark pb-3">
            ...
            <h2 className="text-base font-bold text-white tracking-tight">
              About {creator.name} & Bangkok Travel Focus
            </h2>
          </div>
      ...
      <footer className="border-t border-borderDark bg-surface/60 p-6 text-center text-xs font-mono text-slate-500 mt-auto">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p>© {new Date().getFullYear()} BangkokCams.com • Independent Bangkok Tourism & Streaming Directory</p>
          <div className="flex items-center gap-4">
            <Link href="/?view=map" className="hover:text-slate-300 transition-colors">Live Radar</Link>
            <Link href="/?view=vids" className="hover:text-brandPink transition-colors">BangkokVids</Link>
