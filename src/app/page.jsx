...
export default function AppRoot() {
  ...
  const handleLiveShuffle = useCallback(() => {
    ...
    const liveCams = liveCamsData.filter((c) => {
      ...
      const statusInfo = streamStatus?.entities?.[c.slug] || streamStatus?.entities?.[c.slug ? `creator-${c.slug}` : ''];
      let lat = 13.7469, lng = 100.5349;
      if (c.primary_zone === 'soi_4') { lat = 13.7408; lng = 100.5529; }
      else if (c.primary_zone === 'cowboy') { lat = 13.7371; lng = 100.5613; }
      else if (c.primary_zone === 'khaosan') { lat = 13.7588; lng = 100.4975; }
      else if (c.primary_zone === 'silom') { lat = 13.7285; lng = 100.5342; }
      else if (c.primary_zone === 'chinatown') { lat = 13.7410; lng = 100.5100; }
      else if (c.primary_zone === 'thonglor') { lat = 13.7310; lng = 100.5815; }
      return {
        ...c,
        lat,
        lng,
  ...
  useEffect(() => {
    ...
    window.addEventListener('bangkokcams:select-video', handleSelectCustomVideo);
    return () => {
      window.removeEventListener('bangkokcams:select-video', handleSelectCustomVideo);
    };
  }, []);
            ...
            <div className="bg-surface/90 backdrop-blur-md border border-brandPink/50 shadow-[0_0_24px_rgba(255,42,109,0.3)] rounded-2xl px-4 py-2.5 flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2.5 text-slate-200">
                ...
                <span className="leading-snug">
                  <strong className="text-white font-semibold">Bangkok is resting.</strong> No streams are live right now. Watch latest 4K street walks & nightlife episodes in{' '}
                  <button
