...
export default function ContactModal({ isOpen, onClose, initialTab = 'contact' }) {
  ...
  const handleSendEmail = (e) => {
    e.preventDefault();
    const subject = encodeURIComponent(`[BangkokCams] ${selectedCategoryObj.label} - ${name || 'Inquiry'}`);
    const body = encodeURIComponent(buildMessageBody());
    window.location.href = `mailto:contact@bangkokcams.com?subject=${subject}&body=${body}`;
    setSentNotice(true);
    setTimeout(() => setSentNotice(false), 5000);
  };

  const handleCopy = () => {
    const fullText = `Subject: [BangkokCams] ${selectedCategoryObj.label}\n\n${buildMessageBody()}`;
          ...
          <form onSubmit={handleSendEmail} className="flex flex-col gap-3.5 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text"
                  placeholder="e.g. Billboard Nana or @BangkokWalker"
                  value={name}
            ...
            <div className="pt-2 text-center text-[10px] text-slate-400 border-t border-borderDark/60">
              Direct inbox: <span className="text-brandPink font-mono font-bold">contact@bangkokcams.com</span>
            </div>
          </form>
        )}
          ...
          <div className="flex flex-col gap-3 text-xs text-slate-300 leading-relaxed max-h-[50vh] overflow-y-auto pr-1">
            <div className="p-3 rounded-xl bg-surfaceLight/60 border border-borderDark">
              ...
              <p className="text-slate-400 text-[11px] mb-2">
                BangkokCams uses official YouTube API Services and official embed players to index and display public live broadcasts and video episodes. By using this website, you agree to be bound by the{' '}
                <a
