...
import transitRoutes from '@/public/data/bangkok_transit.json';
...
export default function MapCanvas({ onSelectEntity, onMapInstance, onOpenKohLarn }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedTheme = localStorage.getItem('bangkokcams_map_theme');
  ...
  const handleToggleTheme = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('bangkokcams_map_theme', nextTheme);
    }
  ...
  const populateVenueGroup = useCallback((group, Leaflet, currentStatus) => {
    ...
    activeVenues.forEach((venue) => {
      ...
      marker.bindTooltip(
        ...
        { className: 'bangkok-dark-tooltip', direction: 'top', offset: [0, -14] }
      );
  ...
  useEffect(() => {
    ...
    async function initLeaflet() {
      ...
      const currentTheme = typeof window !== 'undefined'
        ? localStorage.getItem('bangkokcams_map_theme') || 'dark'
        : 'dark';

      // Initialize map centered at Central Bangkok / Siam Junction with rotation support
      const map = Leaflet.map(mapContainerRef.current, {
        center: [13.7469, 100.5349],
        zoom: 13,
        minZoom: 10,
        maxZoom: 18,
        zoomControl: false,
      ...
      // 1. Bangkok Transit Vectors Layer Group (BTS, MRT, Monorails, Waterways)
      const transitGroup = Leaflet.geoJSON(transitRoutes, {
        style: (feature) => {
          const color = feature.properties?.color || '#3B82F6';
          const isWater = feature.properties?.type === 'waterway' || feature.properties?.type === 'canal';
          return {
            color: color,
            weight: isWater ? 4 : 5,
            opacity: 0.95,
            dashArray: isWater ? '6, 6' : undefined,
            lineJoin: 'round',
            lineCap: 'round',
          };
        },
        onEachFeature: (feature, layer) => {
          ...
          const isWater = p.type === 'waterway' || p.type === 'canal';
          const tooltipContent = `
            <div style="font-family: inherit; font-size: 11px;">
              ...
              ${isWater ? '<div style="margin-top: 3px; color: #38BDF8; font-size: 9px; font-weight: 700;">Click to view pier schedule & fares</div>' : ''}
            </div>
          `;
          layer.bindTooltip(tooltipContent, {
            className: 'bangkok-dark-tooltip',
            sticky: true,
            direction: 'top',
          });
      ...
      cctvData.forEach((cam) => {
        ...
        marker.bindTooltip(
          `<div style="font-family: inherit; font-size: 11px;">
             ...
             <span style="display: block; color: #94A3B8; font-size: 10px;">District: ${cam.district || 'Bangkok'}</span>
           </div>`,
          { className: 'bangkok-dark-tooltip', direction: 'top', offset: [0, -6] }
        );
        marker.on('click', () => {
          if (onSelectEntity) {
      ...
      // 4. 24/7 Live Webcams Layer Group (Sukhumvit, Siam & Nightlife Hubs)
      ...
      // 5. Chao Phraya Express & Saen Saep Canal Pier Pins
      const ferryGroup = Leaflet.layerGroup();

      const piers = [
        {
          name: 'Sathorn Pier (Central Pier)',
          sub: 'Chao Phraya Express • BTS Saphan Taksin Interchange',
          fare: '16 THB',
          lat: 13.7192,
          lng: 100.5134,
        },
        {
          name: 'ICONSIAM Pier',
          sub: 'Chao Phraya River • Gold Line Monorail Access',
          fare: 'Free / 16 THB',
          lat: 13.7268,
          lng: 100.5108,
        },
        {
          name: 'Tha Tien Pier (N8)',
          sub: 'Wat Pho & Wat Arun Cross-River Ferry',
          fare: '5 - 16 THB',
          lat: 13.7458,
          lng: 100.4913,
        },
        {
          name: 'Tha Chang Pier (N9)',
          sub: 'Grand Palace & Emerald Buddha Pier',
          fare: '16 THB',
          lat: 13.7523,
          lng: 100.4884,
        },
        {
          name: 'Phra Arthit Pier (N13)',
          sub: 'Khaosan Road River Gateway & Old Town',
          fare: '16 THB',
          lat: 13.7628,
          lng: 100.4939,
        },
        {
          name: 'Pratunam Pier',
          sub: 'Khlong Saen Saep Canal Central Interchange Hub',
          fare: '12-22 THB',
          lat: 13.7498,
          lng: 100.5407,
        },
        {
          name: 'Asok Pier',
          sub: 'Khlong Saen Saep Canal • MRT Phetchaburi Interchange',
          fare: '12-22 THB',
          lat: 13.7495,
          lng: 100.5629,
        },
        {
          name: 'Panfa Leelard Pier',
          sub: 'Khlong Saen Saep Golden Mount Terminal & Old Town',
          fare: '12-22 THB',
          lat: 13.7554,
          lng: 100.5064,
        },
      ...
      piers.forEach((pier) => {
        ...
        marker.bindTooltip(
          `<div style="font-family: inherit; font-size: 11px;">
             <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px;">
               <span style="background: #0891B2; color: #FFFFFF; font-size: 9px; font-weight: 800; padding: 1px 5px; border-radius: 4px;">RIVER / CANAL PIER • ${pier.fare}</span>
             </div>
             ...
             <div style="color: #22D3EE; font-size: 10px; margin-top: 3px; font-weight: 700;">Click to view boat timetables, piers & fares</div>
           </div>`,
          { className: 'bangkok-dark-tooltip', direction: 'top', offset: [0, -14] }
        );
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-canvas">
      ...
      <LayerToggleHUD
        ...
        transitCount={transitRoutes?.features?.length || 0}
        bearing={bearing}
