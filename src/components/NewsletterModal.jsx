...
export default function NewsletterModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      try {
        const subs = JSON.parse(localStorage.getItem('bangkokcams_newsletter_subs') || '[]');
        subs.push({
          email,
          interests,
          date: new Date().toISOString(),
        });
        localStorage.setItem('bangkokcams_newsletter_subs', JSON.stringify(subs));
        localStorage.setItem('bangkokcams_is_subscribed', 'true');
      } catch (err) {
        ...
        {!isSuccess ? (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <span className="text-[10px] font-mono uppercase tracking-widest text-brandPink font-bold">
                  VIP Club • Bangkok Insider
                </span>
                <h3 className="text-lg font-bold text-white leading-tight">
                  Bangkok Pulse VIP Dispatch
                </h3>
              </div>
            </div>

            <p className="text-xs text-slate-300 leading-relaxed mb-4">
              Get notified when iconic venues go live, when new 24/7 webcams are installed, and receive our curated weekend nightlife & transit guide.
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="grid grid-cols-1 gap-1.5 text-xs">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-slate-300 hover:text-white transition-colors">
                    ...
                    />
                    <span>📹 New Street & Traffic CCTV Cams Added</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer text-slate-300 hover:text-white transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span>🎆 Bangkok Festivals, River Fireworks & Transit Updates</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-1.5 text-[10px] text-slate-400 pt-1">
                ...
                <span>Zero spam. 100% Bangkok insider content. Unsubscribe anytime.</span>
              </div>
            </form>
          </div>
        ) : (
          <div className="text-center py-4 space-y-3 animate-fade-in">
            ...
            <p className="text-xs text-slate-300 max-w-xs mx-auto leading-relaxed">
              You are subscribed to Bangkok Pulse VIP Dispatch at <span className="text-amber-300 font-mono">{email}</span>. We will notify you when high-vibe live streams start!
            </p>
            <button
              onClick={onClose}
