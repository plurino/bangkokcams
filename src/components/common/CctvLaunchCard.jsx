...
const BMA_TRAFFIC_URL = 'https://cpudapp.bangkok.go.th/bmatraffic/';

export default function CctvLaunchCard({ cameraCode, cameraType = 'CCTV Node', cameraBrand = 'BMA Traffic' }) {
  ...
  const handleLaunchCityPortal = () => {
    ...
    if (typeof window !== 'undefined') {
      window.open(BMA_TRAFFIC_URL, '_blank', 'noopener,noreferrer');
    }
  };
    ...
    <div className="p-4 rounded-xl bg-cyan-950/20 border border-brandCyan/30 flex flex-col gap-3 shadow-md">
      <div className="flex items-center justify-between">
        <span className="text-xs font-mono text-slate-300 font-bold flex items-center gap-1.5">
          ...
          <span>BMA Traffic CCTV Surveillance Network</span>
        </span>
      ...
      <p className="text-xs text-slate-300 leading-relaxed">
        The Bangkok Metropolitan Administration (BMA) operates municipal traffic surveillance cameras monitoring major arterial intersections, highway interchanges, and expressways. Real-time telemetry is hosted directly on the BMA Traffic portal.
      </p>
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <span>
            {copiedCode
              ? '✓ Code Copied! Opening BMA Traffic Portal...'
              : 'Launch Official BMA Traffic Portal'}
          </span>
          <ExternalLink className="w-4 h-4 shrink-0" />
        </button>
        <span className="text-[10px] text-center text-slate-400 font-mono">
          Auto-copies code <strong className="text-brandCyan">{cameraCode}</strong> and opens <strong className="text-white">cpudapp.bangkok.go.th/bmatraffic/</strong>
        </span>
      </div>
    </div>
  );
