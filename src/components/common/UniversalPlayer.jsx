...
export default function UniversalPlayer({
  ...
  const [clientHost, setClientHost] = useState('bangkokcams.com');
  const playerId = useId();

  useEffect(() => {
    if (typeof window !== 'undefined') {
      setClientHost(window.location.hostname || 'bangkokcams.com');
    }
  }, []);
    ...
    return (
      <div className={`relative w-full aspect-video bg-black rounded-xl overflow-hidden border border-borderDark flex flex-col group shadow-lg ${className}`}>
        <div className="absolute top-2 left-2 z-20 flex items-center gap-1.5 px-2.5 py-1 rounded bg-black/85 backdrop-blur-md border border-cyan-500/30 text-[10px] font-mono text-cyan-400">
          ...
          <span className="font-bold">WINDY WEBCAM • BANGKOK</span>
        </div>
  ...
  if (sourceType === 'twitch' || source?.platform === 'twitch') {
    ...
    const twitchEmbedUrl = `https://player.twitch.tv/?channel=${channel}&parent=${clientHost}&parent=bangkokcams.com&parent=localhost&muted=true&autoplay=true`;
  ...
  const handle = source?.youtube_handle || source?.handle || '@BangkokWalker';
