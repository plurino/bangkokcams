...
const ALERTS_STORAGE_KEY = 'bangkokcams_live_alerts_enabled';
...
export function useLiveAlerts(streamStatus) {
  ...
  const toggleLiveAlerts = async () => {
    ...
    if (Notification.permission === 'granted') {
      ...
      if (next) {
        new Notification('BangkokCams Live Radar', {
          body: '🔔 Live alerts enabled! You will be notified when Bangkok streamers go live.',
    ...
    if (Notification.permission !== 'denied') {
      ...
      if (permission === 'granted') {
        ...
        new Notification('BangkokCams Live Radar', {
          body: '🔔 Subscribed! You will be notified when Bangkok streamers go live.',
          icon: '/images/logo-dark.png',
        });
      }
    } else {
    ...
    Object.entries(entities).forEach(([key, info]) => {
      ...
      if (isNowLive && wasLive === false) {
        try {
          ...
          new Notification('🔴 Live in Bangkok!', {
