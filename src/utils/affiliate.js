/**
 * BangkokCams Affiliate Deep-Link Engine
 * Generates dynamic weekend dates and localized travel affiliate links
 */

const DEFAULT_AGODA_CID = '1894212';
const DEFAULT_12GO_AFFILIATE = 'bangkokcams';
const DEFAULT_AIRALO_CODE = 'BANGKOK10';
...
/**
 * General Bangkok Agoda Search URL (City Code 9395 = Bangkok)
 */
export function buildAgodaBangkokSearchUrl(cid = DEFAULT_AGODA_CID) {
  const { checkin, checkout } = getUpcomingWeekendDates();
  return `https://www.agoda.com/partners/partnersearch.aspx?cid=${cid}&city=9395&checkin=${checkin}&checkout=${checkout}`;
}

/**
 * 12Go Private Airport Transfer Link (BKK/DMK to Bangkok Hotel Door)
 */
export function build12GoTransferUrl(affiliateId = DEFAULT_12GO_AFFILIATE) {
  return `https://12go.asia/en/travel/suvarnabhumi-airport/bangkok?z=${affiliateId}&sub_id=bkk_taxi_card`;
}
...
export function buildAiraloEsimUrl(promoCode = DEFAULT_AIRALO_CODE) {
  return `https://airalo.tp.st/bangkok?referral=${promoCode}&country=thailand`;
}

/**
 * Flight Search URL pre-filled for Bangkok Suvarnabhumi (BKK) / Don Mueang (DMK)
 */
export function buildFlightSearchUrl(departureCode = '', travelDate = '') {
  ...
  if (departureCode && travelDate) {
    return `${base}?origin=${departureCode}&destination=${destination}&depart_date=${travelDate}&marker=bangkokcams`;
  }
  return `https://www.aviasales.com/?destination=${destination}&marker=bangkokcams`;
}
...
export function getKofiTipUrl() {
  return 'https://ko-fi.com/bangkokcams';
}
...
export function getTelegramCommunityUrl() {
  return 'https://t.me/bangkokcams_live';
}
